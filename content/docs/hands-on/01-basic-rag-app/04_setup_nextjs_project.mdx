...
Bun をインストールしていない場合は、以下のコマンドで Bun をインストールする。

```bash title="Terminal"
winget install Oven-sh.Bun
```

bunx.exe の場所を環境変数 Path に追加する。

```txt title="Example of path of bunx.exe"
C:\Users\{ユーザ名}\AppData\Local\Microsoft\WinGet\Packages\Oven-sh.Bun_Microsoft.Winget.Source_8wekyb3d8bbwe\bun-windows-x64
```
